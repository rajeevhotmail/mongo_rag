...
from mongo_store import insert_chunk
...
class EmbeddingsManager:
    ...
    def generate_embeddings(self, batch_size: int = 32) -> Dict[str, Any]:
        ...
        print("✅ Cleared previous data from MongoDB collection.")
        ...
        for i in range(0, len(chunk_texts), batch_size):
            ...
            batch_chunks = self.chunks[i:i+batch_size]  # Get the matching chunk metadata
            ...
            try:
                ...
                batch_docs = []
                for j, (chunk, vector) in enumerate(zip(batch_chunks, batch_embeddings)):
    ...
    def load_vector_db(self) -> bool:
        ...
        try:
            ...
            return False
    def search_similar_chunks_ex(self, query: str, top_k: int = 5) -> List[Dict[str, Any]]:
        """
        Search for chunks similar to the query using MongoDB Atlas Vector Search.
        """
        try:
            ...
            # Run the vector search using Atlas Search
            pipeline = [
                {
                    "$search": {
            ...
            return [
            {
                "chunk": {
                    ...
                    "file_path": result.get("file_path", "unknown"),  # <-- fix the error
            ...
            self.logger.error(f"Error searching MongoDB: {e}", exc_info=True)
            return []
